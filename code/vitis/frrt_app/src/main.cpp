...
int main()
{
	...
	Position_Holonomichp Pos_Goal; //Position of Goal
	...
	for (int i=0; i<4;i++)
	{
		Environment.edges[i].Source=Env.edge[i].source();

		Environment.edges[i].Target=Env.edge[i].target();

		int A=Environment.edges[i].Target.y()-Environment.edges[i].Source.y();
		int B=Environment.edges[i].Target.x()-Environment.edges[i].Source.x();
		Environment.edges[i].AABB=A*A+B*B;
	}

	XTime_GetTime(&sw_processor_start);
	RRT_Star_Dist::Build_RRT_Star( RRT_Star, RRT_Starnb,Pos_Init, Total_Num_Node, R_Robot, Gamma_rrt,
			RRT_Star_Region, Dim, Mu, Environment,x_rand, Verbose);//Build RRT*

	/* for(int i=0;i<Node;i++)
        	cout<<ceil(RRT_Starnb[i].Position.x*100.0)/100.0<<","<<ceil(RRT_Starnb[i].Position.y*100.0)/100.0<<","
        			<<ceil(RRT_Starnb[i].Cost*100.0)/100.0<<endl;*/



	//cout<<"ffi:"<<1<<endl;

	//int Best_Node = RRT_Star_Dist::Best_Trajectory(RRT_Starnb, Pos_Goal, Radio_Goal);
