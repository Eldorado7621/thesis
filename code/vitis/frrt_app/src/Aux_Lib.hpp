...
#define MAX_EDGES 4
	...
	public:
	    ...
	    float squared_distance( Point_2 &p ,int nn)
	    {
		float U = ( ( p.x() - Source.x() ) * ( Target.x() - Source.x() ) +
		    ( p.y() - Source.y() ) * ( Target.y() - Source.y() ) ) / AABB;
		//if(nn<3)
		//  std::cout<<"u"<<U<<","<<p.x()<<","<<Source.x() <<","<<std::endl;

		if( U >= 0 && U <= 1 )
		{
			float mull=( ( Target.x() - Source.x() ) * ( p.y() - Source.y() ) -
					( Target.y() -Source.y() ) * ( p.x() - Source.x() ) );

		    float fm= pow(mull , 2 )/ AABB;
